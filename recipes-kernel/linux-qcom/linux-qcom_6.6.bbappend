...
do_patch:append() {
    ...
    patch -f -p1 < ${WORKDIR}/0002-NOMERGE-drivers-iommu-Bypass-context_irq-error.patch
    patch -f -p1 < ${WORKDIR}/0003-ufs-Configure-UFS-driver-for-NordAU-RUMI.patch
    patch -f -p1 < ${WORKDIR}/0004-ufs-Disable-power-mode-change-during-hba-probe.patch
    patch -f -p1 < ${WORKDIR}/0005-scsi-ufs-Disable-64-bit-DMA-mask.patch
    patch -f -p1 < ${WORKDIR}/0006-QCLINUX-Fix-serial-probe-error.patch
}
